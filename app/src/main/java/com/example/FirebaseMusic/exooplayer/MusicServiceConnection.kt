...
{//mediabrowsercompat을 이용하여 musicService와 연결
   // mediaController를 얻음
    //현재 재생곡, 재생상태,isconnected등 을 livedata로 얻음
    ...
    private inner class MediaBrowserConnectionCallback(private val context: Context) : MediaBrowserCompat.ConnectionCallback()
    {
        ...
        override fun onConnected() {//musicService와 연결되었을때
            Log.d("MusicServiceConnection", "CONNECTED")
            mediaController = MediaControllerCompat(context, mediaBrowser.sessionToken).apply {
                registerCallback(MediaContollerCallback())
            }//mediaController를 얻음
            _isConnected.postValue(Event(Resource.success(true)))//isConnected 할당
        }
